...
export default function ProductPageClient({ initialItems }: { initialItems: FeaturedInventoryItem[] }) {
  ...
  return <main className={styles.main} id="main-content">
    ...
    <article className={styles.product}>
      ...
      <div className={styles.details}><p className={styles.eyebrow}>Available now · {item.condition}</p><h1>{item.title}</h1><p className={styles.byline}>{item.series} · by {item.author}</p><p className={styles.price}>${item.price.toFixed(2)}</p><p className={styles.description}>{item.description}</p><dl><div><dt>Edition</dt><dd>{item.edition}</dd></div><div><dt>Condition</dt><dd>{item.condition}</dd></div><div><dt>Contents</dt><dd>{item.itemCount && item.itemCount > 1 ? `${item.itemCount}-book bundle` : "One physical copy"}</dd></div><div><dt>Fulfillment</dt><dd>Seller → verification hub → buyer</dd></div></dl><AddToCartButton item={{ listingId: item.id, title: item.title, series: item.series, condition: item.condition, price: item.price, imageUrl: item.imageUrl, purchasable: item.purchasable !== false }} /><ul className={styles.assurances}><li><strong>Secure payment</strong><span>Checkout hosted by Stripe</span></li><li><strong>Physical verification</strong><span>Exact copy checked at a selected hub</span></li><li><strong>Controlled seller payment</strong><span>Released after outbound carrier handoff</span></li></ul></div>
    </article>
